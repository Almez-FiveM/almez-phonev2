...
RegisterNUICallback("GetCurrentArrests", function(data, cb)
    ESX.TriggerServerCallback('qb-phone:server:GetCurrentArrests', function(arrests)
        cb(arrests)
    end)
end)

RegisterNUICallback("GetCurrentWeazel", function(data, cb)
    ESX.TriggerServerCallback('qb-phone:server:GetCurrentWeazel', function(weazel)
        cb(weazel)
    end)
end)
...
RegisterNUICallback("GetCurrentJobsCompany", function(data, cb)
    cb(Config.Jobs)
end)

RegisterNUICallback("GetCurrentJobsWorker", function(data, cb)
    ESX.TriggerServerCallback('qb-phone:server:GetCurrentJobsWorker', function(jobworkers)
        cb(jobworkers)
    end, data.JobsJob)
end)

RegisterNUICallback("GetCurrentFoodWorker", function(data, cb)
    ESX.TriggerServerCallback('qb-phone:server:GetCurrentFoodWorker', function(workers)
        cb(workers)
    end, data.FoodJob)
end)
...
AddEventHandler('almez-phone:client:openPhone', function(pData)
        ...
        ESX.TriggerServerCallback('almez-phone:isPhoneRealOwner', function(realowner)
            if realowner ~= false then
                PhoneData.isOwner = true
                print(PhoneData.isOwner)
            else
...
RegisterNUICallback('SendMessage', function(data, cb)
    ...
    if PhoneData.Chats[NumberKey] ~= nil then
        if PhoneData.Chats[NumberKey].messages[ChatKey] ~= nil then
            ...
            TriggerServerEvent('qb-phone:saveMessageToDatabase', PhoneData.Chats[NumberKey].messages, ChatNumber)
...
local kameraAcik = false
RegisterNUICallback('KameraAc', function(data, cb)
    SetNuiFocus(false, false)
    SetNuiFocusKeepInput(false)
    CreateMobilePhone(1)
    ...
    while kameraAcik do
        ...
        if IsControlJustPressed(1, 27) then -- Toogle Mode
            onCam = not onCam
            CellFrontCamActivate(onCam)

        else if IsControlJustPressed(1, 176) then
         exports['screenshot-basic']:requestScreenshotUpload('https://discord.com/api/webhooks/703020462732738630/lo9NptMpZu-aTLK410Qmlp5fOW2TqzFxzX5u69KB3DyzKLjyE0tXqeBP8st1ykvb_bad', 'files[]', function(data2)
            DestroyMobilePhone()
            CellCamActivate(false, false)
            local resp = json.decode(data2)
            test2 = resp.attachments[1].proxy_url
...
RegisterNUICallback("DeleteAlarm", function(data)
    print(PhoneData.Alarms[tonumber(data.id) + 1])
...
RegisterNUICallback("WhatsappOptions", function(data, cb)
    ...
    TriggerServerEvent("qb-phone:WhatsappActions", data)
end)
...
AddEventHandler('qb-phone:client:GiveContactDetails', function()
    ...
    if player ~= -1 and distance < 2.5 then
        ...
        if lastServerId ~= PlayerId then
            ...
            exports.mythic_notify:SendAlert('error', 'Bu kişiye iletişim bilgilerini zaten verdin.')
        end
    else
...
RegisterNUICallback('GetCurrentpolices', function(data, cb)
    ESX.TriggerServerCallback('qb-phone:server:GetCurrentpolices', function(polices)
        cb(polices)
    end)
end)

RegisterNUICallback("GetCurrentDrivers", function(data, cb)
    ESX.TriggerServerCallback('qb-phone:server:GetCurrentDrivers', function(drivers)
        cb(drivers)
    end)
end)

RegisterNUICallback("GetCurrentMecano", function(data, cb)
    ESX.TriggerServerCallback('qb-phone:server:GetCurrentMecano', function(mecano)
        cb(mecano)
    end)
end)

RegisterNUICallback("GetCurrentDoctor", function(data, cb)
    ESX.TriggerServerCallback('qb-phone:server:GetCurrentDoctor', function(doctor)
        cb(doctor)
    end)
end)
...
RegisterNetEvent('qb-phone:client:anon')
AddEventHandler('qb-phone:client:anon',function(msj)
...
RegisterCommand('realownertest', function()
    ESX.TriggerServerCallback('almez-phone:isPhoneRealOwner', function(realowner)
        print(realowner)
        if realowner ~= false then
            exports['mythic_notify']:SendAlert('success', 'Gercek sahip sensin!')
        else
            exports['mythic_notify']:SendAlert('error', 'Gercek sahip degilsin!')
        end
    end)
end)

RegisterCommand('mailtest', function()
    TriggerServerEvent("qb-phone:server:sendNewMail", { sender = "almez", subject = " almez", message = "Deniyoz bisler", messageUrl = "https://cdn.discordapp.com/attachments/783410820604362763/849692449692712970/unknown.png"})
end)

RegisterCommand('mailtest2', function()
    TriggerServerEvent("qb-phone:server:sendNewMail", { sender = "almez2", subject = " qwe", message = "qwe", messageUrl = "https://media.discordapp.net/attachments/848342020057858098/852004950623911977/mqdefault.png"})
end)