...
$(document).on('click', '.twitter-header-tab', function (e) {
    ...
    if (PressedTwitterTab !== CurrentTwitterTab) {
        ...
        if (PressedTwitterTab === "twitter-mentions") {
            $.post('http://almez-phonev2/ClearMentions');
        }
        ...
        if (PressedTwitterTab == "twitter-self") {
            $.post('http://almez-phonev2/GetSelfTweets', JSON.stringify({}), function (selfdata) {
...
$(document).on('click', '.tweet-delete', function (e) {
    ...
    // console.log(id)
    $.post('http://almez-phonev2/DeleteTweet', JSON.stringify({
        id: id,
    }));

    $.post('http://almez-phonev2/GetSelfTweets', JSON.stringify({}), function (tweets) {
...
$(document).on('click', '#send-tweet', function (e) {
    ...
    if (TweetMessage != "" || TweetUrl != "") {
        ...
        $.post('http://almez-phonev2/PostNewTweet', JSON.stringify({
...
$(document).on('click', '.hashtag-tag-text', function (e) {
    ...
    if (!HashtagOpen) {
        ...
        $.post('http://almez-phonev2/GetHashtagMessages', JSON.stringify({
            hashtag: Hashtag
