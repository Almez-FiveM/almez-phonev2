...
$(document).on('click', '#join-race', function(e){
    ...
    $.post('http://almez-phonev2/IsInRace', JSON.stringify({}), function(IsInRace){
        if (!IsInRace) {
            $.post('http://almez-phonev2/RaceDistanceCheck', JSON.stringify({
                RaceId: Data.RaceId,
                Joined: true,
            }), function(InDistance){
                if (InDistance) {
                    $.post('http://almez-phonev2/IsBusyCheck', JSON.stringify({
                        check: "editor"
                    }), function(IsBusy){
                        if (!IsBusy) {
                            $.post('http://almez-phonev2/JoinRace', JSON.stringify({
                                RaceData: Data,
                            }));
...
$(document).on('click', '#quit-race', function(e){
    ...
    $.post('http://almez-phonev2/LeaveRace', JSON.stringify({
        RaceData: Data,
    }));
...
$(document).on('click', '#start-race', function(e){
    ...
    $.post('http://almez-phonev2/StartRace', JSON.stringify({
        RaceData: Data,
    }));
...
$(document).on('click', '.dropdown .dropdown-menu li', function(e) {
    $.post('http://almez-phonev2/GetTrackData', JSON.stringify({
        RaceId: $(this).attr('id')
    }), function(TrackData){
...
$(document).on('click', '#setup-race', function(e){
    ...
    $.post('http://almez-phonev2/GetRaces', JSON.stringify({}), function(Races){
...
$(document).on('click', '#create-race', function(e){
    e.preventDefault();
    $.post('http://almez-phonev2/IsAuthorizedToCreateRaces', JSON.stringify({}), function(data){
...
$(document).on('click', '#racing-create-accept', function(e){
    ...
    if (TrackName !== "" && TrackName !== undefined && TrackName !== null) {
        $.post('http://almez-phonev2/IsAuthorizedToCreateRaces', JSON.stringify({
            TrackName: TrackName
        }), function(data){
            if (data.IsAuthorized) {
                if (data.IsNameAvailable) {
                    $.post('http://almez-phonev2/StartTrackEditor', JSON.stringify({
                        TrackName: TrackName
                    }));
...
$(document).on('click', '#setup-race-accept', function(e){
    ...
    $.post('http://almez-phonev2/HasCreatedRace', JSON.stringify({}), function(HasCreatedRace){
        if (!HasCreatedRace) {
            $.post('http://almez-phonev2/RaceDistanceCheck', JSON.stringify({
                RaceId: track,
                Joined: false,
                ...
                if (InDistance) {
                    if (track !== undefined || track !== null) {
                        if (laps !== "") {
                            $.post('http://almez-phonev2/CanRaceSetup', JSON.stringify({}), function(CanSetup){
                                if (CanSetup) {
                                    $.post('http://almez-phonev2/SetupRace', JSON.stringify({
                                        RaceId: track,
                                        AmountOfLaps: laps,
                                    }))
                                    $(".racing-overview").animate({
...
$(document).on('click', '#leaderboards-race', function(e){
    ...
    $.post('http://almez-phonev2/GetRacingLeaderboards', JSON.stringify({}), function(Races){
        if (Races !== null) {
