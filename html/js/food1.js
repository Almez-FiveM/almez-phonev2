    ...
    $.post('http://almez-phonev2/GetCurrentFoodWorker', JSON.stringify({
        ...
        if (status.length > 0) {
            $.each(status, function(i, food){
                var element = '<div class="food-list" id="foodid-'+i+'"> <div class="food-list-firstletter">'+ '</div> <div class="food-list-fullname">' + food.firstname + " " + food.lastname + '</div> <div class="food-list-call"><i class="fas fa-phone"></i></div> </div>'
                $(".food2-list").append(element);
                $("#foodid-"+i).data('foodData', food);
            });
