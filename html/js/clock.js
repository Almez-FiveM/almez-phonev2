...
$(document).on('click', '.clock-app-account', function(e){
    ...
    MI.Phone.Notifications.Add("fas fa-university", MI.Phone.Functions.Lang("CLOCK_TITLE"), "IBAN Kopyalandı!", "#badc58", 1750);
});


$(document).on('click', '#clear-clock', function(event){
    $(".transactions-item-div").remove()
    MI.Phone.Notifications.Add("fas fa-university", MI.Phone.Functions.Lang("CLOCK_TITLE"), "Transfer geçmişin temizlendi!", "#badc58", 1750);
    console.log("clearing clock data")
    $.post("http://almez-phonev2/ClearclockData");
})
...
$(document).on('click', '#new-alarm', function(e){
    e.preventDefault();
    MI.Phone.Animations.TopSlideDown(".clock-app-transfer", 400, 0);
});
...
$(document).on('click', '.alarm-item > i', function(e){
    ...
    var id = $(this).data("id")
    console.log(id)
    $.post("http://almez-phonev2/DeleteAlarm", JSON.stringify({id: id}));
    $("#alarm-item-"+ id).animate({
        "margin-left": -50+"vh"
    }, 350, function(){
        $("#alarm-item-"+id).remove();
        MI.Phone.Notifications.Add("far fa-clock", MI.Phone.Functions.Lang("CLOCK_TITLE"), "Alarm kaldırıldı!", "#ff7e1496", 1750); 
    });
});
...
$(document).on('click', '.clock-app-header-button', function(e){
    ...
    if (CurrentTab != PressedTab) {
        ...
        // }
        console.log(CurrentTab, PressedTab)
        $(".clock-app-"+CurrentTab).animate({
            left: 30+"vh"
        }, 250, function(){
            $(".clock-app-"+CurrentTab).css({"display":"none"})
...
MI.Phone.Functions.DoclockOpen = function(data) {
...
$(document).on('click', '.clock-app-transfer-button', function(e){
    ...
    if (type == "accept-alarm") {
        ...
        $.post("http://almez-phonev2/SetAlarm", JSON.stringify({clock: value}));
        $.post('http://almez-phonev2/GetAlarmData', JSON.stringify({}), function(data){
            MI.Phone.Functions.DoclockOpen(data);
        });
        MI.Phone.Notifications.Add("far fa-clock", MI.Phone.Functions.Lang("CLOCK_TITLE"), "Saat " + value + " için alarm kurdun!", "color: #ff7e1496;", 1500);
    }
    setTimeout(() => {
        MI.Phone.Animations.TopSlideUp(".clock-app-transfer", 400, -100) 
    }, 100);
})
