...
$(document).on('click', '#accept-transfer', function(e){
    ...
    if (iban != "" && amount != "" && amount > 0) {
        if (amountData >= amount) {
            ...
            $.post('http://almez-phonev2/TransferMoney', JSON.stringify({
                iban: iban,
                amount: amount
            }), function(data){
                if (data.CanTransfer) {
                    ...
                    MI.Phone.Notifications.Add("fas fa-university", "subzBank", "Şu kadar; "+amount+",- $ para transfer edildi!", "#badc58", 1500);
                } else {
                    // // console.log(117)
...
$(document).on('click', '.pay-invoice', function(event){
    ...
    if (BankBalance >= InvoiceData.amount) {
        $.post('http://almez-phonev2/PayInvoice', JSON.stringify({
...
$(document).on('click', '.decline-invoice', function(event){
    ...
    $.post('http://almez-phonev2/DeclineInvoice', JSON.stringify({
        sender: InvoiceData.sender,
        amount: InvoiceData.amount,
